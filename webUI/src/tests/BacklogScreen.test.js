...
describe('Backlog Component', () => {
  ...
  test('divides data into sprintData and backlogData correctly', async () => {
    ...
    await waitFor(()=>{
      expect(screen.getAllByTestId('backlog-data-box')).toHaveLength(1);
    })
    
  });

  test('moves task from sprintData to Backlog correctly after drag and drop', async () => {
    // Mock data for issues and sprints
    const mockData = [
      {
        id: '7883168',
        title: 'GET Api Call',
    ...
    // Find a task in sprintData and Backlog
    const taskInSprintData = screen.getByText('sprintData');
    const taskInBacklog = screen.getByText('Backlog');

    // Perform drag and drop simulation
    fireEvent.dragStart(taskInSprintData);
    fireEvent.dragEnter(taskInBacklog);
    fireEvent.dragOver(taskInBacklog);
    fireEvent.drop(taskInBacklog);

    // Ensure that the task has moved from sprintData to Backlog
    await waitFor(() => {
      expect(taskInSprintData).not.toBeInTheDocument();
